"""
Parallel Pipeline Agent 3: Gelsenkirchen, Braunschweig, Aachen, Kiel, Chemnitz, Halle
"""
import asyncio, re, csv, httpx
from pathlib import Path
from playwright.async_api import async_playwright

CITIES = ['Gelsenkirchen', 'Braunschweig', 'Aachen', 'Kiel', 'Chemnitz', 'Halle (Saale)']
OUT_DIR = Path(r'C:\Users\steph\.qclaw-oversea\workspace\projects\chase\zahnarzte-leads\paralysis3')
OUT_DIR.mkdir(parents=True, exist_ok=True)
CSV_OUT = OUT_DIR / 'leads_agent3.csv'

BAD_EMAILS = {'noreply','no-reply','datenschutz','privacy','info@','kontakt@','praxis@','service@','support@','team@','office@','mail@','blzk@','kennstdueinen','jameda','usercentrics','sentry.io','sentry-next','domain.com','google.com','lzk','kzvr','lzkth','aekwl','zaek-sh','zaeksh','bezreg','brd.nrw','sozmi.landsh','docinsider','jimdo','webnode','wix.com','cookie','consent','borlabs','tracking','marketing','poststelle','redaktion','webmaster@','presse@','redaktion@','datenschutzbeauftragte','kzvr.de','kzvs','zaek-sh','zaeksh','aeksh','zaekwl','kzvwl','bezreg-koeln'}
...
async def main():
    ...
    with open(CSV_OUT,'w',newline='',encoding='utf-8') as f:
        ...
        w.writeheader()
        w.writerows(results)
    print(f'Done. CSV: {CSV_OUT} | Clean emails: {sum(1 for r in results if r["email"])}')
