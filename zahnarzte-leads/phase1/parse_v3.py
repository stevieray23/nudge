"""
Parse Gelbe Seiten dentist listings — v3.
Fix: Walk back MORE aggressively past street names and address components.
The order is: NAME → category → [street] → postal+city → [district] → distance
We need to skip: street lines (no postal code, no rating, no category, no tier)
"""
...
OUTPUT_PATH = Path(r"C:\Users\steph\.qclaw-oversea\workspace\projects\chase\zahnarzte-leads\phase1\agent1_frankfurt_bremen.json")

# Lines that should NEVER be treated as the clinic name
BAD_TOKENS = {
    'Zahnärzte', 'Zahnärztinnen',
    'Webseite', 'Geöffnet', 'Geschlossen',
...
def is_bad_name(line: str) -> bool:
    s = line.strip()
    if not s: return True
    if s in BAD_TOKENS: return True
    if any(s.startswith(p) for p in BAD_PREFIXES): return True
    if re.match(r'^\d,\d\s+\d+\.\d+$', s): return True  # rating
    if re.match(r'^\d+[,\.]\d+\s*km$', s): return True  # distance
    if re.match(r'^\d{5}\s', s): return True  # postal code line
    return False

def looks_like_street_name(line: str) -> bool:
    """True if this line looks like a street address, not a clinic name."""
    s = line.strip()
    if not s: return True
    # Any line starting with a capital letter followed by a mix of letters/spaces
    # but not containing 'Dr.' or 'Praxis' or 'Zahn' (typical dental terms)
    # Streets often have number suffixes
    if re.match(r'^[A-ZÄÖÜ][a-zäöüß\s\-]+[\d\-\.,]+$', s): return True
    if re.match(r'^[A-ZÄÖÜ][a-zäöüß\s\-]+$', s) and re.search(r'\d', s): return True
    # Common German street suffix words
    street_suffixes = ('str.', 'straße', 'weg', 'platz', 'ring', 'berg', 'hof',
                      'allee', 'tor', 'park', 'graben', 'wall', 'markt', 'hof',
                      'gasse', 'pfad', 'steig', 'kamp', 'feld', 'born', 'bach',
                      'brunnen', 'garten', 'hecke', 'heck', 'busch', 'wald')
    lower = s.lower()
    if any(lower.endswith(sfx) for sfx in street_suffixes): return True
    return False

def find_clinic_name_before_addr(lines, addr_start_idx: int) -> str | None:
    """
    Starting from addr_start_idx (the postal-code line), walk BACKWARDS
    up to 6 lines. Skip: tier labels, category, street name, ratings.
    Return the first good name found.
    """
    # First candidate is the line BEFORE the postal code line (the street)
    # Then we need to skip that and go further back
    # Strategy: keep going back until we hit a non-street, non-meta line
    # that looks like a clinic name

    for offset in range(1, 8):  # 1=street line, 2=name, 3=category, etc.
        idx = addr_start_idx - offset
        if idx < 0:
            break
        ...
        if is_bad_name(candidate):
            continue
        if looks_like_street_name(candidate):
            continue  # skip street names
        # This should be the clinic name
        return candidate

    # Fallback: try without street-skip (just take first non-bad)
    for offset in range(1, 6):
        idx = addr_start_idx - offset
        if idx < 0: break
        candidate = lines[idx].strip()
        if not candidate: continue
        if is_bad_name(candidate): continue
        return candidate

    return None


def build_addr_lines(lines, addr_start_idx: int) -> list[str]:
    """Collect address lines starting from the postal code line."""
    addr_lines = []
    j = addr_start_idx
    while j < len(lines):
        nxt = lines[j].strip()
        if not nxt:
            break
        # Stop conditions
        if re.match(r'^\d,\d\s+\d+\.\d+$', nxt): break  # rating
        if re.match(r'^\d+[,\.]\d+\s*km$', nxt): break  # distance
        if nxt in ('Webseite', 'Geöffnet', 'Geschlossen'): break
        if nxt.startswith('–'): break
        # Another postal code = new listing
        if j > addr_start_idx and re.match(r'^\d{5}\s', nxt): break
        # A tier label mid-block
...
def parse_blocks(raw_text: str, city: str) -> list[dict]:
    ...
    while i < len(lines):
        ...
        if re.match(r'^\d{5}\s', line):
            # Found postal code line — start of address block
            addr_start_idx = i
            addr_lines = build_addr_lines(lines, addr_start_idx)
            name = find_clinic_name_before_addr(lines, addr_start_idx)
            ...
            i = addr_start_idx + len(addr_lines)
        else:
            i += 1
...
def main():
    # Paste the raw text buffers from the source pages
    # (These are assigned in the full script below)
    all_records = []
    for city, raw in CITY_DATA:
        recs = parse_blocks(raw, city)
    ...
    for r in all_records:
        key = (r['clinic_name'].strip(), r['address'].strip())
        if key not in seen:
            seen.add(key)
            deduped.append(r)
    ...
    city_counts = Counter(r['city'] for r in deduped)
    print("\nPer-city breakdown:")
    for c, cnt in sorted(city_counts.items()):
        print(f"  {c}: {cnt}")


# ─── CITY RAW DATA ────────────────────────────────────────────────────────────
# Each block is the extracted text from a Gelbe Seiten dentist search page.

CITY_DATA = []

# === FRANKFURT ===
FRANKFURT_RAW = open(r"C:\Users\steph\.qclaw-oversea\workspace\projects\chase\zahnarzte-leads\phase1\frankfurt_raw.txt", encoding="utf-8").read() if False else """
Platin Partner
...
CITY_DATA.append(("Frankfurt am Main", FRANKFURT_RAW))

# === BREMEN ===
CITY_DATA.append(("Bremen", """
Business
Boronowsky Stephan
 Zahnärzte
Am Markt 11,
...
Hagmanngarten 2,
 ...
 7,2 km
 Webseite
Darwish Darwi.Dent B. Dr. med. dent. Zahnarztpraxis
 Zahnärzte
Rüttenscheider Str. 134,
 45131 Essen
 (Rüttenscheid)
 ...
 – Öffnet um 14:00
 Webseite
Dyckhoff Carl-Michael Zahnarzt
 Zahnärzte
Kuglerstr. 24,
 45144 Essen
 (Frohnhausen)
 ...
 – Öffnet um 14:30
 Webseite
Fürst Peter Dr.med.dent. Zahnarzt
 Zahnärzte
Klapperstr. 38,
 45277 Essen
 (Überruhr-Hinsel)
 ...
 Geschlossen
 – Öffnet um 14:00
Grotkamp Frank-Ludger Dr. Zahnarzt
 Zahnärzte
Am Schwarzen 7,
 45239 Essen
 (Heidhausen)
 ...
 – Öffnet um 14:30
 Webseite
Klauer Matthias Dr. u. Juliane Dr. Zahnärzte
 Zahnärzte
Altenessener Str. 401,
 45326 Essen
 (Altenessen-Süd)
 4,4 km
Voß Torsten Dr. Zahnarzt
 5,0 5.0
 2 Bewertungen
 Zahnärzte
Heidehang 2,
 ...
 – Öffnet um 14:00
 Webseite
Zadeh Talayeh Dr. Zahnarztpraxis
 Zahnärzte
Steeler Str. 258,
 45138 Essen
 (Huttrop)
...
Huestr. 18,
 ...
 – Öffnet um 15:00
 Webseite
Przybylek Christoph Dr. / Przybylek Thomas
 Zahnärzte: Implantologie (Schwerpunkt)
Markstr. 417,
 44795 Bochum
 (Weitmar)
...
Wanner Str. 7,
 ...
 – Öffnet um 14:00
 Webseite
Dr. Przybylek + Kollegen Zahnarztpraxis
 Zahnärzte
Hellweg 62,
 58455 Witten
 (Heven)
