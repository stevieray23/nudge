"""
Robust parser for Gelbe Seiten dentist listings.
Approach: Find lines containing German postal codes (5 digits), then walk backwards to find the clinic name.
"""
...
# Tiers and other markers to skip when looking for a name
SKIP_NAMES = {
    'Platin Partner', 'Gold Partner', 'Bronze Partner', 'Silver Partner',
    'Premium Partner', 'Basic Partner', 'Top Partner', 'Silber Partner',
    'Business', 'Economy', 'Aus der Region', 'Häufige Fragen',
    # categories
    'Zahnärzte', 'Zahnärztinnen',
    # actions / status
    'Webseite', 'Geöffnet', 'Geschlossen',
}
SKIP_PREFIXES = ('– ', 'Schließt um', 'Öffnet um', 'nächste Seite')

def is_bad_name(line: str) -> bool:
    """Return True if this line should NOT be treated as a clinic name."""
    stripped = line.strip()
    if not stripped:
        return True
    ...
    if stripped.startswith('–'):
        return True
    # Rating patterns like "5,0 5.0" or "4,9 4.9" 
    if re.match(r'^\d,\d\s+\d+\.\d+$', stripped):
        return True
    # Just "Bewertungen" or "Bewertung"
    if stripped in ('Bewertungen', 'Bewertung'):
        return True
    # Distance only
    if re.match(r'^\d+[,\.]\d+\s*km$', stripped):
        return True
    # Postal code line (shouldn't be a name)
...
def parse_blocks(raw_text: str, city: str) -> list[dict]:
    """
    Split raw text into listing blocks. Each block ends at a line containing a postal code.
    Then extract: name (previous good line), address (postal code line + next 1-2 lines).
    """
    ...
    while i < len(lines):
        ...
        # Is this a postal code line? That's the start of an address block.
        if re.match(r'^\d{5}\s', line):
            # Build address from this line and following lines until we hit a non-address line
            addr_lines = [line]
            j = i + 1
            while j < len(lines):
                nxt = lines[j]
                # Stop on: rating, tier label, another postal code, status line, webseite
                if re.match(r'^\d,\d\s+\d+\.\d+$', nxt):  # rating
                    break
                if re.match(r'^\d+[,\.]\d+\s*km$', nxt):  # distance
                    break
                if nxt in ('Webseite', 'Geöffnet', 'Geschlossen'):
                    break
                if nxt.startswith('–'):
                    break
                if re.match(r'^\d{5}\s', nxt):  # another postal code = new block
                    break
                if nxt in SKIP_NAMES or nxt.startswith(('Platin Partner','Gold Partner','Bronze Partner','Premium Partner','Basic Partner','Silber Partner','Top Partner','Business','Economy','Aus der Region')):
                    break
                # Also stop if next line looks like a new name (no address content)
                if not nxt[0].isdigit() and not nxt.startswith(('(',)):
                    # Check if it's a name candidate
                    if not is_bad_name(nxt) and not re.match(r'^\d', nxt):
                        # Could be a continuation of address (street name)
            ...
            # Now walk backwards from i to find the clinic name
            name = None
            for k in range(i - 1, max(-1, i - 6), -1):
                candidate = lines[k]
                if is_bad_name(candidate):
                    continue
                # Also skip if it starts with digit
