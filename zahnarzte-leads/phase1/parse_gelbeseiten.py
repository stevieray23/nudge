"""
Parse Gelbe Seiten dentist listings from web_fetch text content.
Parses the raw extracted text from web_fetch and builds structured JSON.
"""
...
# ─── RAW TEXT BUFFERS ─────────────────────────────────────────────────────────
# Frankfurt (page 1) — ~561 total results
FRANKFURT_RAW = """
Platin Partner
...
# Bremen (page 1) — ~202 total results
BREMEN_RAW = """
Business
Boronowsky Stephan
 Zahnärzte
...
# Essen (page 1) — ~335 total results
ESSEN_RAW = """
...
# Bochum (page 1) — ~277 total results
BOCHUM_RAW = """
Taschke, Bettina Dr.
 4,9 4.9
 51 Bewertungen
...
# Bielefeld (page 1) — ~145 total results
BIELEFELD_RAW = """
Business
Praxis Dr. Geu
 5,0 5.0
...
def parse_listings(raw_text: str, city: str) -> list[dict]:
    """
    Parse the extracted Gelbe Seiten text into structured records.
    Strategy: The page format is repeating blocks of:
      1. optional tier label (Platin Partner, Gold Partner, etc.)
      2. name (first non-empty non-tier line)
      3. address lines (lines that look like postal code + city)
      4. optional Webseite link marker
    """
    records = []
    ...
    while i < len(lines):
        ...
        # Skip tier labels and section headers
        if line in ('Platin Partner', 'Gold Partner', 'Bronze Partner', 'Premium Partner',
                    'Basic Partner', 'Business', 'Economy', 'Aus der Region',
                    'Häufige Fragen', 'Zahnarzt in Frankfurt am Main (561 Treffer)',
                    'Zahnarzt in Bremen (202 Treffer)', 'Zahnarzt in Essen (335 Treffer)',
                    'Zahnarzt in Bochum (277 Treffer)', 'Zahnarzt in Bielefeld (145 Treffer)',
                    'Zahnarzt in Bonn (221 Treffer)', 'Silber Partner', 'Top Partner'):
            i += 1
            continue

        # Skip lines that are clearly not names
        skip_words = ('Bewertung', 'Bewertungen', 'Zahnärzte', 'Webseite',
                      'Geöffnet', 'Geschlossen', 'Öffnet um', 'Schließt um')
        if any(line.startswith(s) for s in skip_words):
            i += 1
            continue

        # Skip numeric rating lines like "5,0 5.0" or "4,9 4.9"
        if re.match(r'^\d,\d\s+\d+\.\d+$', line):
            i += 1
            continue

        # Skip distance indicators like "3,6 km"
        if re.match(r'^\d+[,\.]\d+\s*km$', line):
            i += 1
            continue

        # Skip postal code lines (don't start with them)
        if re.match(r'^\d{5}\s', line):
            i += 1
            continue

        # Skip non-address lines that are clearly metadata
        ...
        # Look ahead for address — first line that contains a 5-digit postal code
        address_parts = []
        j = i + 1
        addr_found = False
        while j < len(lines) and len(address_parts) < 4:
            candidate = lines[j]
            # Postal code pattern: starts with 5 digits
            if re.match(r'^\d{5}\s', candidate):
                # Collect this and any following address lines
                addr_found = True
                addr_start = j
                addr_lines = []
                k = j
                while k < len(lines) and len(addr_lines) < 3:
                    # Keep collecting while we have address-like content
                    # Stop when we hit rating, distance, geöffnet/geschlossen, or another name
                    nxt = lines[k]
                    if re.match(r'^\d+[,\.]\d+\s+\d+\.\d+$', nxt):
                        break
                    if re.match(r'^\d+[,\.]\d+\s*km$', nxt):
                        break
                    if nxt in ('Geöffnet', 'Geschlossen') or nxt.startswith('– Öffnet') or nxt.startswith('– Schließt'):
                        break
                    if re.match(r'^\d{5}\s', nxt) and k > j:
                        break
                    addr_lines.append(nxt.strip())
                    k += 1
                address_parts = addr_lines
                break
            j += 1
        ...
        if address_parts:
            ...
            # Extract phone placeholder (not available in listing overview)
            phone = ""
            website = ""
...
def main():
    ...
    city_data = [
        ("Frankfurt am Main", FRANKFURT_RAW),
        ("Bremen", BREMEN_RAW),
        ("Essen", ESSEN_RAW),
        ("Bochum", BOCHUM_RAW),
        ("Bielefeld", BIELEFELD_RAW),
        ("Bonn", BONN_RAW),
    ]

    for city, raw in city_data:
        recs = parse_listings(raw, city)
        print(f"  {city}: {len(recs)} records parsed")
        all_records.extend(recs)

    print(f"\nTotal: {len(all_records)} records")

    # Deduplicate by (clinic_name, address)
    seen = set()
    deduped = []
