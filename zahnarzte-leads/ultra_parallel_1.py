"""
Ultra-fast city scraper - Group 1 (10 cities, 3 parallel browsers)
"""
import asyncio, json, re, csv, time, httpx
from pathlib import Path
from playwright.async_api import async_playwright

CITIES = ['Frankfurt am Main','Bremen','Essen','Bochum','Bielefeld','Bonn','Münster','Mannheim','Karlsruhe','Augsburg']
BASE = Path(r'C:\Users\steph\.qclaw-oversea\workspace\projects\chase\zahnarzte-leads')
OUT = BASE / 'v2_batch1.json'
SEEN_FILE = BASE / 'v2_seen_websites.json'

# Load already-seen websites to avoid duplicates
seen_websites = set()
if SEEN_FILE.exists():
...
PLATFORMS = ['golocal','stadtbranchenbuch','kennstdueinen','dastelefonbuch','dasoertliche','11880','jameda','docinsider','dentalk','dental','zahni']
...
def is_personal(e):
    ...
    lo = e.lower()
    if any(b in lo for b in PLATFORMS): return False
    lp = lo.split('@')[0]
    # Accept if it looks like a personal name email
...
async def crawl_one(client, url, name):
    async with SEM:
        ...
        return clean_name(name)[0], clean_name(name)[1], ''
            ...
            try:
                ...
                for al in all_links:
                    ...
                    if href and href.startswith('http') and 'gelbeseiten' not in href.lower() and not is_platform_url(href):
                        bad = ['facebook','instagram','linkedin','bing','google','yahoo','apple','docinsider','jimdo','webnode','golocal','stadtbranchenbuch','kennstdueinen','dastelefonbuch','11880','dasoertliche']
                        if any(b in href.lower() for b in bad): continue
                        if any(k in txt for k in ['website','homepage','zur web','onlineshop']) and not is_platform_url(href):
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(15.0, connect=5.0), limits=httpx.Limits(max_connections=60)) as client:
        ...
        print(f'Total emails: {sum(1 for _,_,_,e in results if e)}')
    ...
    with open(SEEN_FILE, 'w', encoding='utf-8') as f:
        json.dump(list(seen_websites | {r['website'] for r in all_recs}), f)
    print(f'Updated seen_websites: {len(seen_websites)} -> {len(seen_websites)+len(all_recs)}')
